...
|-------|----------------|-----------|----------------|------------|-------------------|-------|
| Claude Fable 5 | `claude-fable-5` / `fable` | Frontier; hardest, longest-running, most ambiguous work | 1M | 128k | Yes (always on) | GA June 9 2026. Opt-in, never the default. Routes flagged cyber/bio requests to Opus 4.8. [^1] |
| Claude Opus 4.8 | `claude-opus-4-8` / `opus` | Default capable Opus tier; complex reasoning + agentic coding | 1M | 128k | Yes (only thinking mode) | What `opus` resolves to on the Anthropic API. [^2] |
| Claude Sonnet 4.6 | `claude-sonnet-4-6` / `sonnet` | Balanced daily driver | 1M | 64k | Yes | Best speed/intelligence balance. [^2] |
| Claude Haiku 4.5 | `claude-haiku-4-5` / `haiku` | Fastest; high-throughput subagents | 200k | 64k | No | Near-frontier intelligence at the lowest tier; built for fan-out workers. [^2] |

Three facts in that table catch people over and over, so it is worth slowing down on each.

The first is that Haiku 4.5 carries a 200k window while every other current model carries 1M. Fan heavy work out to a swarm of Haiku subagents and you cannot hand each one a million-token slice; the window simply is not there to fill. [^3]

The second is that Haiku 4.5 sits out adaptive thinking and the effort parameter entirely. Set effort against it and the control becomes a silent no-op. Nothing errors, nothing warns, the dial just spins free. [^4]

The third is that Opus 4.8 has a single thinking mode and that mode is adaptive. The legacy `thinking: {type: "enabled", budget_tokens: N}` comes back as a 400 error. The full deprecation gradient lives in section 5.5. [^5]

### Fable 5 and the Mythos research tier

Fable 5 is the frontier model, built for tasks larger than a single sitting: multi-day, goal-directed, ambiguous problems where the model investigates before it acts and verifies its own work more than its smaller siblings do. It is not the Claude Code default on any account type. You reach for it deliberately with `/model fable`. [^6]
...
Confirm the active model anytime via `/status` (which also shows account info) or a configured status line. The current effort level shows next to the spinner, for example "with low effort". [^17]
...
Chains cap at three models after dedup; the switch holds for the current turn only, then the primary gets another shot on your next message. `--fallback-model` beats the `fallbackModel` setting, and `"default"` expands to your default model inside a chain. [^18]

## 5.3 Provider-dependent alias resolution -- read this before you trust `opus`

The single biggest source of "why is my model different from yours" confusion comes down to one fact: aliases resolve to different versions depending on your provider and account type. The word `opus` does not mean the same thing on every desk in the room. [^19]
...
And the `default` model by account type: [^20]
...
On Fable 5, effort is the primary intelligence, latency, and cost dial. Start at `high`, the default, push to `xhigh` for the most capability-sensitive work, and drop to `medium` or `low` for routine tasks. The detail that resets your instincts: lower effort on Fable 5 often exceeds `xhigh` on prior models, so you can run it cheaper than its predecessors and still come out ahead. Reduce effort if a task finishes correctly but drags. [^29]

On Sonnet 4.6 the docs nudge against the default. It ships at `high`, but the recommendation is to set `medium` explicitly as the practical default for most apps -- agentic coding, tool-heavy workflows -- to avoid the unexpected latency that `high` invites. Drop to `low` for chat and high-volume work; climb to `high` or `max` only when quality dominates speed and cost. [^30]
...
```
Note `effort` lives under `output_config` and thinking under `thinking`. They are separate parameters that work together, and on Opus 4.8 thinking is off unless you set `thinking: {type: "adaptive"}` explicitly. [^34]
...
Something flipped on the 4.5 and 4.6 generations, and most of modern prompting follows from it. These models are proactive, literal, and capable enough that the failure mode reversed direction. The old prompts were written to fight under-doing, full of "CRITICAL: you MUST use this tool", "be thorough", "go above and beyond". Run those same prompts on current models and you get over-triggering, over-engineering, and over-delegation instead. The work now is mostly subtraction: remove the coaching, and add the why. [^46]

### Core principles, which carry across every model

The first principle is to be explicit about action versus suggestion. "Can you suggest changes..." often yields suggestions; "Change this function to improve performance" yields edits. The models follow instructions literally, so say what you mean. [^47]

The second, and the single highest-ROI habit, is to explain the why and not just the what. "NEVER use ellipses" is weaker than "Your response is read aloud by a text-to-speech engine, so never use ellipses since the engine will not know how to pronounce them." The model generalizes from the rationale. [^48]

The third is to tell it what to do, not what to avoid. "Your response should be composed of smoothly flowing prose paragraphs" beats "do not use markdown". [^49]

The fourth is to dial back the anti-laziness language. Where you said "CRITICAL: You MUST use this tool when...", now say "Use this tool when...". Replace "Default to using [tool]" with "Use [tool] when it would enhance your understanding of the problem." Remove "if in doubt, use [tool]" entirely, because it over-triggers now. [^50]
...
```
When parallel bash starts bottlenecking your own machine, dial the other way with the inverse one-liner the docs supply: "Execute operations sequentially with brief pauses between each step to ensure stability." [^54]
...
First-window and later-window prompts want to read differently. Spend the first window scaffolding: write the tests, lay down an `init.sh` setup script, build the runway. Later windows just iterate against the todo list. And when you can, prefer starting fresh over compaction, because these models are genuinely good at rediscovering their own state from the filesystem -- running `pwd`, reading `progress.txt`, `tests.json`, and the git log to find their place again. Hand it verification tools, Playwright MCP or computer use, so a long autonomous run can check its own work without you sitting there to confirm it. [^56]
...
Fable 5 follows instructions well enough that a brief instruction does the work an enumerated list used to do, and the over-prescriptive skills you wrote for older models can actively drag its output down. When you adopt it, re-audit your skills and prune the scaffolding you no longer need; Fable 5 will even update skills on the fly based on what it learns from the task at hand. [^58]

Describe the outcome, not the steps. Hand Fable 5 the result you want and let it plan the path; pair it with `/goal` to keep it working until the outcome holds. Skip the verification reminders, since it self-verifies with less prompting. Size up: give it work you would normally break into pieces, and start at the top of your difficulty range rather than testing it on easy tasks that undersell its range. [^59]

Hand it ambiguity. Root-cause investigations, outage debugging, and architecture decisions are where the extra investigation pays off. [^60]

Expect longer turns by default. Hard tasks can run many minutes per request, and autonomous runs can stretch for hours. Adjust client timeouts, streaming, and progress indicators before you migrate, and consider async or scheduled check-ins rather than blocking until each run returns. [^61]
...
- Claude platform docs -- *Models overview* (lineup, specs, context windows, max output, tokenizer note, legacy table, deprecation warning). [^67]
- Claude platform docs -- *Effort* (levels, per-model support, recommended effort by model, ultracode note). [^68]
- Claude platform docs -- *Adaptive thinking* (adaptive vs manual vs disabled, supported models, display defaults, pricing, Fable 5 reasoning extraction). [^69]
- Claude platform docs -- *Prompting best practices* (the proactivity inversion, the sample prompts, prefill migration). [^70]
- Claude platform docs -- *Prompting Claude Fable 5* (scaffolding changes, longer turns, grounding progress claims, early stopping). [^71]
- Claude platform docs -- *Pricing* (live per-MTok and 1M-window numbers). [^72]
- Claude platform docs -- *Model deprecations* (retirement dates and migration targets). [^73]
- Claude Code docs -- *Model configuration* (`/model`, aliases, `opusplan`, fallback chains, provider resolution, effort, ultracode, ultrathink, extended thinking, extended context, third-party pinning). [^74]
- Claude Code docs -- *Changelog* (version floors and per-feature minimums; re-verify version-pinned claims here). [^75]

[^1]: web \ | 2026-06-18 \ | [platform.claude.com](https://platform.claude.com) models overview, "Claude Fable 5 and Claude Mythos 5" table (GA date, specs); code.claude.com model-config, "Automatic model fallback" (routing to Opus 4.8)
[^2]: web \ | 2026-06-18 \ | [platform.claude.com](https://platform.claude.com) models overview, "Latest models comparison" table
[^3]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) models overview, "Latest models comparison" table
[^4]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Adjust effort level" -- "Models not listed here do not support effort"
[^5]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "Supported models"
[^6]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Work with Fable 5"
[^7]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Automatic model fallback" + "Check what triggered fallback"
[^8]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) models overview, "Claude Fable 5 and Claude Mythos 5" + Info callout
[^9]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) models overview, "Legacy models" table; platform.claude.com effort, "supported by" Note
[^10]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) models overview, "Legacy models" Warning callout
[^11]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) models overview, context-window tooltip: "the same text produces roughly 30% more tokens"
[^12]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) models overview, Note: "Every Claude model ID is a pinned snapshot"
[^13]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Model aliases" table
[^14]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "opusplan model setting"
[^15]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "opusplan model setting" -> advisor tool
[^16]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Setting your model"
[^17]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Checking your current model" + "Set the effort level"
[^18]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Fallback model chains"
[^19]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Model aliases" resolution paragraph
[^20]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "default model setting"
[^21]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Pin models for third-party deployments"
[^22]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, Note: "Opus 4.8 requires Claude Code v2.1.154 or later" + "Work with Fable 5" Note
[^23]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Customize pinned model display and capabilities"
[^24]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Restrict model selection"
[^25]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) effort, "How effort works" + "Effort with tool use"
[^26]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) effort, "Effort levels" table + "When to adjust the effort parameter"
[^27]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Adjust effort level" table + "calibrated per model"; platform.claude.com effort, "supported by" Note
[^28]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) effort, "Recommended effort levels for Claude Opus 4.7" + "...4.8"
[^29]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) effort, "Recommended effort levels for Claude Fable 5"
[^30]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) effort, "Recommended effort levels for Sonnet 4.6"
[^31]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Set the effort level" + "Adjust effort level"
[^32]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) effort, "Claude Code's ultracode mode" Note; code.claude.com model-config, "Choose an effort level" -- ultracode row
[^33]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "How adaptive thinking works" + intro Tip
[^34]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "Adaptive thinking with the effort parameter" code sample + "Supported models"
[^35]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "Adaptive vs manual vs disabled thinking" table + "Supported models" Warning
[^36]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Adaptive reasoning and fixed thinking budgets"
[^37]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Use ultrathink for one-off deep reasoning"
[^38]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Leverage thinking" Note: "Claude Opus 4.5 is particularly sensitive to the word think"
[^39]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "Tuning thinking behavior"
[^40]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "Pricing" -- output_tokens_details.thinking_tokens
[^41]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "Controlling thinking display" + display-default Note; code.claude.com model-config, "Extended thinking"
[^42]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "Cost control"
[^43]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) adaptive-thinking, "Thinking output on Claude Fable 5 and Claude Mythos 5"
[^44]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Extended context"
[^45]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Extended context" table + pricing paragraph
[^46]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Migration considerations" #6 + "Tool usage" overtrigger paragraph
[^47]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Tool usage" example
[^48]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Add context to improve performance"
[^49]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Control the format of responses" #1
[^50]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Overthinking and excessive thoroughness" + "Tool usage"
[^51]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Minimizing hallucinations in agentic coding"
[^52]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Overeagerness" sample prompt
[^53]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Avoid focusing on passing tests and hard-coding"
[^54]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Optimize parallel tool calling" -- both sample prompts
[^55]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Subagent orchestration" sample prompt
[^56]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Long-horizon reasoning and state tracking" + "Multi-context window workflows"
[^57]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Balancing autonomy and safety" sample prompt
[^58]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) prompting-claude-fable-5, "Strong instruction following" + "Recommended scaffolding changes"
[^59]: web | 2026-06-18 | [code.claude.com](https://code.claude.com) model-config, "Work with Fable 5"; platform.claude.com prompting-claude-fable-5, "Recommended scaffolding changes"
[^60]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) prompting-claude-fable-5, "Capability improvements" + "Recommended scaffolding changes"
[^61]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) prompting-claude-fable-5, "Longer turns by default"
[^62]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) prompting-claude-fable-5, "Ground progress claims during long runs"
[^63]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) prompting-claude-fable-5, "Strong instruction following" + "Readability when communicating with the user"
[^64]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) prompting-claude-fable-5, "Rare cases of early stopping"
[^65]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Frontend design" -- frontend_aesthetics sample prompt
[^66]: web | 2026-06-18 | [platform.claude.com](https://platform.claude.com) claude-prompting-best-practices, "Migrating away from prefilled responses"
[^67]: web | 2026-06-18 | [https://platform.claude.com/docs/en/about-claude/models/overview](https://platform.claude.com/docs/en/about-claude/models/overview)
[^68]: web | 2026-06-18 | [https://platform.claude.com/docs/en/build-with-claude/effort](https://platform.claude.com/docs/en/build-with-claude/effort)
[^69]: web | 2026-06-18 | [https://platform.claude.com/docs/en/build-with-claude/adaptive-thinking](https://platform.claude.com/docs/en/build-with-claude/adaptive-thinking)
[^70]: web | 2026-06-18 | [https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices)
[^71]: web | 2026-06-18 | [https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5)
[^72]: web | 2026-06-18 | [https://platform.claude.com/docs/en/about-claude/pricing](https://platform.claude.com/docs/en/about-claude/pricing)
[^73]: web | 2026-06-18 | [https://platform.claude.com/docs/en/about-claude/model-deprecations](https://platform.claude.com/docs/en/about-claude/model-deprecations)
[^74]: web | 2026-06-18 | [https://code.claude.com/docs/en/model-config](https://code.claude.com/docs/en/model-config)
[^75]: web | 2026-06-18 | [https://code.claude.com/docs/en/changelog.md](https://code.claude.com/docs/en/changelog.md)
