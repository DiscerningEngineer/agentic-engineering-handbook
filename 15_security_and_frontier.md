...
- **Native Windows:** not supported. Run inside WSL2. WSL1 is not supported, because bubblewrap needs kernel features only WSL2 provides. [^4]
- **Ubuntu 24.04+ gotcha:** the default AppArmor policy blocks bubblewrap from creating the user namespaces it needs. Check with `sysctl kernel.apparmor_restrict_unprivileged_userns`; if it returns `1`, add an AppArmor profile for `/usr/bin/bwrap` (the docs give the exact `/etc/apparmor.d/bwrap` snippet, then `sudo systemctl reload apparmor`). [^4]
...
- **Blocked:** shell config such as `~/.bashrc`, system binaries in `/bin/`, and anything outside cwd and temp, without explicit permission. [^4]
...
> The sandbox auto-denies writes to Claude Code's own `settings.json` at every scope and to the managed-settings directory, so a sandboxed command cannot rewrite its own policy. [^4]
...
> **Subprocess caveat.** Claude Code does not pass `OTEL_*` to subprocesses (Bash, hooks, MCP, language servers). An OTel-instrumented app you run via Bash will not inherit the exporter endpoint, so set those vars in the command itself. [^11]
...
**Managed settings file locations** (deploy via MDM): [^9]
...
> **`.mcp.json` is a supply-chain surface.** It is committed to the repo, so a PR can add an MCP server. Anthropic does not security-audit MCP servers, so review `.mcp.json` like code and gate it with the MCP allow/deny keys above. (Detail in the MCP chapter.) [^7]
...
Before you deploy this, verify each key name and the `network` sub-keys against the current settings reference. The sandbox network keys especially are version-sensitive. [^4] [^9]
...
Here is the through-line to carry out of the chapter. Sandboxing is necessary and genuinely excellent at shrinking the blast radius, with Anthropic's own number being an 84 percent cut in prompt fatigue alongside more safety, not less. The limits prove the other half of the sentence, in Anthropic's own words: it reduces risk and it is not a complete isolation boundary. The senior's job is to assemble independent layers, configure them for the real trust level of the code sitting in front of you right now, and keep an audit trail the agent has no way to influence. That assembly is judgment, and judgment is the thing the cohort is selling. [^3] [^4]

---

## Sources

- Claude Code docs -- *Sandboxing* (OS primitives, platform support, filesystem and network pillars, modes, the escape hatch and strict mode, the configuration footguns, and the "reduces risk but is not a complete isolation boundary" plus domain-fronting framing). [^4]
- Claude Code docs -- *Permission modes* (mode matrix, `bypassPermissions` behavior, auto-mode availability, the circuit breaker, protected paths, conversational boundaries). [^8]
- Claude Code docs -- *Monitoring usage* (OTel metric, event, and trace names, attributes, env vars, the privacy/cardinality tiers, and collector guidance). [^11]
- Claude Code docs -- *Settings* (managed-settings keys, file locations, precedence, drop-in merge order, tolerant-parsing fail-closed behavior, permission-rule grammar). [^9]
- Claude Code docs -- *Security* (SOC 2 and ISO 27001 pointer, VM-for-untrusted-content guidance, `ConfigChange` hook, `.mcp.json` supply-chain surface, MCP-not-audited). [^7]
- Claude Code docs -- *Hooks* (the security-and-compliance hook subset and lifecycle events). [^10]
- Claude Code docs -- *Authentication* (Enterprise SSO, domain capture, RBAC, Compliance API, `apiKeyHelper` refresh). [^20]
- Claude Code docs -- *Agent View* (supervisor behavior, sleep vs shutdown, `claude respawn --all`, `disableAgentView`). [^16]
- Claude Code docs -- *Scheduled tasks* (`/loop` vs Routines vs `/goal`). [^17]
- Claude Code docs -- *Changelog* (version floors and per-feature minimums; re-verify version-pinned claims here). [^21]
- Anthropic Engineering -- *Native sandboxing in Claude Code* (84% prompt reduction, the two-pillar framing). [^3]
- Anthropic Engineering -- *Claude Code auto mode* (93% prompt-approval stat, prompt-injection definition, classifier mechanics, 0.4% FPR / 17% FNR / 5.7% synthetic FNR). [^1]
- Claude Platform docs -- *Managed Agents: Dreams* (Dreams API mechanics, limits, lifecycle, supported models, transcript archival). [^18]
- Claude blog -- *New in Claude Managed Agents* (Harvey ~6x figure, the review-before-land control). [^19]
- Claude support -- *Covered models under a BAA* (Claude API is an Eligible Service while Claude Code is not, plus the ZDR-blocks-Covered-Models nuance). [^13]
- Claude support -- *HIPAA-ready Enterprise plans* (covered scope and the bundled-seat Claude-Code-not-covered caveat). [^14]
- Claude support -- *Anthropic certifications* (HIPAA-ready, ISO 27001, ISO 42001, SOC 2 Type I and II, Trust Center). [^15]
- NVD -- *CVE-2025-66479* (sandbox-runtime network non-enforcement when no allowed domains configured, fixed in v0.0.16). [^6]
- Ona -- *How Claude Code escapes its own denylist and sandbox* (Leonardo Di Donato, 2026-03-03; the `/proc/self/root` path bypass and the self-disabling-sandbox reasoning). [^5]

[^1]: web | 2026-06-18 | [https://www.anthropic.com/engineering/claude-code-auto-mode](https://www.anthropic.com/engineering/claude-code-auto-mode)
[^2]: web \ | 2026-06-18 \ | [https://www.anthropic.com/engineering/claude-code-auto-mode](https://www.anthropic.com/engineering/claude-code-auto-mode)
[^3]: web | 2026-06-18 | [https://www.anthropic.com/engineering/claude-code-sandboxing](https://www.anthropic.com/engineering/claude-code-sandboxing)
[^4]: web | 2026-06-18 | [https://code.claude.com/docs/en/sandboxing](https://code.claude.com/docs/en/sandboxing)
[^5]: web | 2026-06-18 | [https://ona.com/stories/how-claude-code-escapes-its-own-denylist-and-sandbox](https://ona.com/stories/how-claude-code-escapes-its-own-denylist-and-sandbox)
[^6]: web | 2026-06-18 | [https://nvd.nist.gov/vuln/detail/CVE-2025-66479](https://nvd.nist.gov/vuln/detail/CVE-2025-66479)
[^7]: web | 2026-06-18 | [https://code.claude.com/docs/en/security](https://code.claude.com/docs/en/security)
[^8]: web | 2026-06-18 | [https://code.claude.com/docs/en/permission-modes](https://code.claude.com/docs/en/permission-modes)
[^9]: web | 2026-06-18 | [https://code.claude.com/docs/en/settings](https://code.claude.com/docs/en/settings)
[^10]: web | 2026-06-18 | [https://code.claude.com/docs/en/hooks](https://code.claude.com/docs/en/hooks)
[^11]: web | 2026-06-18 | [https://code.claude.com/docs/en/monitoring-usage](https://code.claude.com/docs/en/monitoring-usage)
[^12]: web \ | 2026-06-18 \ | [https://code.claude.com/docs/en/authentication](https://code.claude.com/docs/en/authentication)
[^13]: web | 2026-06-18 | [https://support.claude.com/en/articles/15455031-covered-models-under-a-business-associate-agreement-baa](https://support.claude.com/en/articles/15455031-covered-models-under-a-business-associate-agreement-baa)
[^14]: web | 2026-06-18 | [https://support.claude.com/en/articles/13296973-hipaa-ready-enterprise-plans](https://support.claude.com/en/articles/13296973-hipaa-ready-enterprise-plans)
[^15]: web | 2026-06-18 | [https://support.claude.com/en/articles/10015870-what-certifications-has-anthropic-obtained](https://support.claude.com/en/articles/10015870-what-certifications-has-anthropic-obtained)
[^16]: web | 2026-06-18 | [https://code.claude.com/docs/en/agent-view](https://code.claude.com/docs/en/agent-view)
[^17]: web | 2026-06-18 | [https://code.claude.com/docs/en/scheduled-tasks](https://code.claude.com/docs/en/scheduled-tasks)
[^18]: web | 2026-06-18 | [https://platform.claude.com/docs/en/managed-agents/dreams](https://platform.claude.com/docs/en/managed-agents/dreams)
[^19]: web | 2026-06-18 | [https://claude.com/blog/new-in-claude-managed-agents](https://claude.com/blog/new-in-claude-managed-agents)
[^20]: web | 2026-06-18 | [https://code.claude.com/docs/en/authentication](https://code.claude.com/docs/en/authentication)
[^21]: web | 2026-06-18 | [https://code.claude.com/docs/en/changelog.md](https://code.claude.com/docs/en/changelog.md)
